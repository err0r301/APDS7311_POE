...
module.exports.register = async (req, res) => {
    const { username, name,surname, accountNumber, IDNumber, password } = req.body;
    ...
    if (!passwordRegex.test(password)) {
        return res.status(400).send('Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one digit, and one special character');
    }
    ...
    if (!username || typeof username !== 'string' || !surnameRegex.test(username)) {  
